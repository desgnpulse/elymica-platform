...
export function LoginForm() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (response?.error || !response?.ok) {
      setError('Unable to sign in. Confirm your school code and credentials.');
      return;
    }
  ...
  return (
    <form
      ...
      className="space-y-6 rounded-2xl border border-sage/20 bg-white/95 p-8 shadow-xl shadow-sand/40 backdrop-blur"
    >
      <div>
        <p className="text-xs uppercase tracking-[0.5em] text-olive">Teacher access</p>
        <h1 className="mt-2 font-heading text-3xl text-night">Deliver world-class lessons</h1>
        <p className="mt-2 text-sm text-olive">
          Sign in with the credentials shared by your school admin to manage classes.
        </p>
      </div>
